...
Charts are configured for responsiveness and include download options.
"""
...
# Shared Plotly layout overrides for a polished, consistent look
...
# Plotly config enabling PNG download, fullscreen, and responsive
CHART_CONFIG = {
    "displayModeBar": True,
        ...
        if operation in {"filter", "list", "greater_than", "less_than", "between"} and _is_small_table(table):
            ...
            if cat_col:
                ...
                fig = px.pie(
                    ...
                    title=f"Distribution of {cat_col} in results", hole=0.35,
                )
                return _apply_layout(fig)
            if numeric_cols:
                fig = px.histogram(
